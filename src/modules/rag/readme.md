...
- 文書埋め込みとクエリ埋め込みを分け、モデル差し替え時の互換性を保つ
...
- `embedder_factory.py` で設定に応じた埋め込み実装を選択する
- `sentence_transformer_embedder.py` で `bge-m3` などのローカル埋め込みモデルを呼び出す
...
- `services/embedder.py` を RAG 側の唯一の埋め込み境界として扱う
- `infrastructure/embedder_factory.py` で設定から具体実装を選ぶ
- 既定値は `sentence_transformer` + `BAAI/bge-m3` にする
- ローカル開発や依存未導入時の退避先として `hash` 実装も残す
- 将来 `qdrant` を実接続する際は、`Embedder.dimensions` を使って collection 設計へつなげる
